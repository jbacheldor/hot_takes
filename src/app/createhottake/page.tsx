...
function HotTakes({
  ...
  hotTakeId: string;
    ...
    const insertHotTake = () => {
      fetch('http://localhost:3000/server/inserthottake', {method: 'POST', body: JSON.stringify(formState)})
  ...
  return (
      <div className="hot-container">
          ...
          <div className={"hot-body"}>
              <label>hot take<input name='hot_take' value={formState?.hot_take}
                                    onChange={onChangeHandler}></input></label>
              <label>first name<input name='first_name'
                                      value={formState?.first_name}
                                      onChange={onChangeHandler}></input></label>
              <label>last name<input name='last_name' value={formState?.last_name}
                                    onChange={onChangeHandler}></input></label>
